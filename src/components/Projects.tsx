            ...
            <div className="relative rounded-[15px] bg-orange-50/90 backdrop-blur-sm overflow-hidden flex flex-col lg:flex-row">
                <div className="relative lg:w-3/5">
                    <img
                        ...
                        onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/800x600/ffedd5/f97316?text=IMAGE+N/A'; }}
                    />
                </div>
                <div className="p-8 lg:w-2/5 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {project.tech.map((tech: string) => (
                            <span key={tech} className="px-2 py-1 bg-orange-500/10 text-orange-700 rounded-md text-xs border border-orange-500/20">
                                {tech}
                            </span>
                        ))}
                    </div>
                    <div className="flex space-x-4 mt-auto pt-4 border-t border-orange-200/50">
                        <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-gray-600 hover:text-orange-600 transition-colors duration-300">
                            <Github size={18} /><span>Code</span>
                        </a>
                        <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-gray-600 hover:text-orange-600 transition-colors duration-300">
...
export default function Projects() {
  const projects = [
    {
      ...
      image: "https://placehold.co/800x600/f97316/ffffff?text=VR+Assessment", // Placeholder for local image
      github: "#",
      live: "#",
    },
    {
    ...
    {
      title: "Freaky Hollow",
      description: "A narrative-driven horror game prototype with atmospheric level design and scripted events to build suspense.",
  ...
  return (
    <section id="projects" className="py-20 relative overflow-hidden bg-orange-50">
      <GridPattern />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-16">
            ...
            <div>
                <h3 className="text-2xl font-mono text-orange-600 mb-4 pl-4 border-l-4 border-orange-500">[ Featured Project ]</h3>
            ...
            {otherProjects.length > 0 && (
                <div>
                    <h3 className="text-2xl font-mono text-orange-600 mb-4 pl-4 border-l-4 border-orange-500">[ Other Projects ]</h3>
                            ...
                            {otherProjects.map((project, index) => (
                                <button
                                    ...
                                    className={`relative text-left w-full px-4 py-3 text-sm font-medium transition-colors duration-200 rounded-md whitespace-nowrap ${
                                        selectedProjectIndex === index ? 'text-orange-700' : 'text-gray-500 hover:bg-orange-100/50 hover:text-orange-600'
                                    }`}
                                >
                                    {selectedProjectIndex === index && (
                                        <motion.div layoutId="active-tab-highlight" className="absolute inset-0 bg-gradient-to-r from-orange-200/80 to-amber-200/80 rounded-md" />
                                    )}
                                ...
                                >
                                    <div className="md:w-1/2">
                                        <img
                                            ...
                                            onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/600x400/ffedd5/f97316?text=IMAGE+N/A'; }}
                                        />
                                    </div>
                                    <div className="md:w-1/2 flex flex-col">
                                        <p className="text-gray-600 text-sm leading-relaxed mb-4 flex-grow">{otherProjects[selectedProjectIndex].description}</p>
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {otherProjects[selectedProjectIndex].tech.map((tech: string) => (
                                                <span key={tech} className="px-2 py-1 bg-orange-500/10 text-orange-700 rounded-md text-xs border border-orange-500/20">{tech}</span>
                                            ))}
                                        </div>
                                        <div className="flex space-x-4 mt-auto pt-4 border-t border-orange-200/50">
                                            <a href={otherProjects[selectedProjectIndex].github} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-gray-600 hover:text-orange-600 transition-colors">
                                                <Github size={18} /><span>Code</span>
                                            </a>
                                            <a href={otherProjects[selectedProjectIndex].live} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-gray-600 hover:text-orange-600 transition-colors">
                                                <ExternalLink size={18} /><span>Live Demo</span>
                                            </a>
                                        </div>
                                    </div>
